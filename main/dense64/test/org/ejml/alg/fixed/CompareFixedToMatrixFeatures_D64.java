...
// TODO this should be improved by creating custom matrices for many of these functions.
    // TODO also should share code with CompareFixedToCommonOps
public abstract class CompareFixedToMatrixFeatures_D64 {
    ...
    public CompareFixedToMatrixFeatures_D64(Class classFixed) {
        ...
        N = Integer.parseInt(classFixed.getSimpleName().charAt(13)+"");
    }
    ...
    public void compareToCommonOps() {
        ...
        for( Method fixedM : methods ) {
            ...
            Method commonM = null;
            for( Method m : MatrixFeatures_D64.class.getMethods()) {
                if( isMatch(fixedM,m)) {
                    commonM = m;
                    break;
                }
...
//                System.out.println("not matched: "+fixedM.getName());
    ...
    private void declareParamStandard(Class[] typesFixed, Object[] inputsFixed, Object[] inputsCommon) {
        for( int i = 0; i < typesFixed.length; i++ ) {
            if(FixedMatrix_F64.class.isAssignableFrom(typesFixed[i])) {
                ...
                RowMatrix_F64 m = RandomMatrices_D64.createRandom(f.getNumRows(), f.getNumCols(), rand);

                ConvertMatrixStruct_F64.convert(m, f);
                inputsFixed[i] = f;
                inputsCommon[i] = m;
            } else if( double.class == typesFixed[i] ) {
                inputsFixed[i] = UtilEjml.TEST_F64;
                inputsCommon[i] = UtilEjml.TEST_F64;
    ...
    private boolean checkEquivalent( Object a , Object b ) {
        if( a == null ) {
            ...
            return MatrixFeatures_D64.isIdentical(m, bb, UtilEjml.TEST_F64);
