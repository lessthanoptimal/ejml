 ...
 * @author Peter Abeles
 */
public class TestSingularOps_R64 {
    ...
    private void testDescendingOrder(int numRows, int numCols, boolean compact, boolean testArray ) {
        ...
        if( compact ) {
            U = SimpleMatrix.wrap(RandomMatrices_R64.createOrthogonal(numRows,minLength,rand));
            W = SimpleMatrix.wrap(RandomMatrices_R64.createDiagonal(minLength,minLength,0,1,rand));
            V = SimpleMatrix.wrap(RandomMatrices_R64.createOrthogonal(numCols,minLength,rand));
        } else {
            U = SimpleMatrix.wrap(RandomMatrices_R64.createOrthogonal(numRows,numRows,rand));
            W = SimpleMatrix.wrap(RandomMatrices_R64.createDiagonal(numRows,numCols,0,1,rand));
            V = SimpleMatrix.wrap(RandomMatrices_R64.createOrthogonal(numCols,numCols,rand));
        }
        ...
        if( testArray ) {
            SingularOps_R64.descendingOrder(U.matrix_F64(),false,singularValues,minLength,V.matrix_F64(),false);
            // put back into W
            for( int i = 0; i < singularValues.length; i++ )
                W.set(i,i,singularValues[i]);
        } else {
            SingularOps_R64.descendingOrder(U.matrix_F64(),false,W.matrix_F64(),V.matrix_F64(),false);
        }
    ...
    private void testDescendingInputTransposed(int numRows, int numCols,
        ...
        U = SimpleMatrix.wrap(RandomMatrices_R64.createOrthogonal(numRows,minLength,rand));
        S = SimpleMatrix.wrap(RandomMatrices_R64.createDiagonal(minLength,minLength,0,1,rand));
        V = SimpleMatrix.wrap(RandomMatrices_R64.createOrthogonal(numCols,minLength,rand));
        ...
        // put into descending order
        if( testArray ) {
            SingularOps_R64.descendingOrder(U.matrix_F64(),tranU,singularValues,minLength,V.matrix_F64(),tranV);
            // put back into S
            for( int i = 0; i < singularValues.length; i++ )
                S.set(i,i,singularValues[i]);
        } else {
            SingularOps_R64.descendingOrder(U.matrix_F64(),tranU,S.matrix_F64(),V.matrix_F64(),tranV);
        }
    ...
    public void descendingOrder_NaN() {
        ...
        SingularOps_R64.descendingOrder(U.matrix_F64(),false,S.matrix_F64(),V.matrix_F64(),false);
    ...
    private void checkSvdMatrixSize_positive( int numRows , int numCols )
    {
        ...
        DMatrixRow_F64 U = new DMatrixRow_F64(numRows,numRows);
        DMatrixRow_F64 W = new DMatrixRow_F64(numRows,numCols);
        DMatrixRow_F64 V = new DMatrixRow_F64(numCols,numCols);

        SingularOps_R64.checkSvdMatrixSize(U,false,W,V,false);
        CommonOps_R64.transpose(U);
        CommonOps_R64.transpose(V);
        SingularOps_R64.checkSvdMatrixSize(U,true,W,V,true);

        // compact SVD
        U = new DMatrixRow_F64(numRows,s);
        W = new DMatrixRow_F64(s,s);
        V = new DMatrixRow_F64(numCols,s);
        ...
        SingularOps_R64.checkSvdMatrixSize(null,false,W,null,false);
        SingularOps_R64.checkSvdMatrixSize(null,true,W,V,true);
        SingularOps_R64.checkSvdMatrixSize(U,true,W,null,true);
    }
    ...
    private void checkSvdMatrixSize_negative( int numRows , int numCols )
    {
        ...
        // create a none compact SVD
        DMatrixRow_F64 U = new DMatrixRow_F64(numRows,s);
        DMatrixRow_F64 W = new DMatrixRow_F64(numRows,numCols);
        DMatrixRow_F64 V = new DMatrixRow_F64(numCols,s);
    ...
    public void nullVector() {
        for( int numRows = 2; numRows < 10; numRows++ ) {
            for( int numCols = 2; numCols < 10; numCols++ ) {
                ...
                SimpleMatrix A = SimpleMatrix.wrap(RandomMatrices_R64.createRandom(numRows,numCols,rand));
                ...
                SimpleMatrix v = SimpleMatrix.wrap(SingularOps_R64.nullVector(svd, true , null));
                ...
                v = SimpleMatrix.wrap(SingularOps_R64.nullVector(svd, false , null));
    ...
    public void nullSpace() {
        for( int numRows = 2; numRows < 5; numRows++ ) {
            for( int numCols = 2; numCols < 5; numCols++ ) {
                ...
                SimpleMatrix ns = SimpleMatrix.wrap(SingularOps_R64.nullSpace(svd,null,UtilEjml.EPS));

                // make sure the null space is not all zero
                assertTrue( Math.abs(CommonOps_R64.elementMaxAbs(ns.matrix_F64())) > 0 );
                ...
                assertTrue( Math.abs(CommonOps_R64.elementMaxAbs(found.matrix_F64())) <= 10*UtilEjml.EPS );
            }
        }
    }
    ...
    public void rank_and_nullity(){
        ...
        //wide matrix
        A = new DMatrixRow_F64(1,3,true,1,0,0);
        rank_and_nullity(A,1,2);

        // tall matrix
        A = new DMatrixRow_F64(3,1,true,1,0,0);
        rank_and_nullity(A,1,0);
    }

    public void rank_and_nullity(DMatrixRow_F64 A , int rank , int nullity ) {
        SingularValueDecomposition_F64<DMatrixRow_F64> alg = DecompositionFactory_R64.svd(A.numRows,A.numCols,true,true,false);
        assertTrue(alg.decompose(A));

        assertEquals(rank, SingularOps_R64.rank(alg, UtilEjml.EPS));
        assertEquals(nullity, SingularOps_R64.nullity(alg, UtilEjml.EPS));
    }
    ...
    public void rank_and_nullity_noArgument(DMatrixRow_F64 A , int rank , int nullity ) {
        SingularValueDecomposition_F64<DMatrixRow_F64> alg = DecompositionFactory_R64.svd(A.numRows,A.numCols,true,true,false);
        assertTrue(alg.decompose(A));

        assertEquals(rank, SingularOps_R64.rank(alg));
        assertEquals(nullity, SingularOps_R64.nullity(alg));
    }
}
