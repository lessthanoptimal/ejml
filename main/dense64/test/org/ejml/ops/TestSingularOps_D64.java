 ...
 * @author Peter Abeles
 */
public class TestSingularOps_D64 {
    ...
    private void testDescendingOrder(int numRows, int numCols, boolean compact, boolean testArray ) {
        ...
        if( compact ) {
            U = SimpleMatrix.wrap(RandomMatrices_D64.createOrthogonal(numRows,minLength,rand));
            W = SimpleMatrix.wrap(RandomMatrices_D64.createDiagonal(minLength,minLength,0,1,rand));
            V = SimpleMatrix.wrap(RandomMatrices_D64.createOrthogonal(numCols,minLength,rand));
        } else {
            U = SimpleMatrix.wrap(RandomMatrices_D64.createOrthogonal(numRows,numRows,rand));
            W = SimpleMatrix.wrap(RandomMatrices_D64.createDiagonal(numRows,numCols,0,1,rand));
            V = SimpleMatrix.wrap(RandomMatrices_D64.createOrthogonal(numCols,numCols,rand));
        }
    ...
    private void testDescendingInputTransposed(int numRows, int numCols,
        ...
        U = SimpleMatrix.wrap(RandomMatrices_D64.createOrthogonal(numRows,minLength,rand));
        S = SimpleMatrix.wrap(RandomMatrices_D64.createDiagonal(minLength,minLength,0,1,rand));
        V = SimpleMatrix.wrap(RandomMatrices_D64.createOrthogonal(numCols,minLength,rand));
    ...
    private void checkSvdMatrixSize_positive( int numRows , int numCols )
    {
        ...
        SingularOps_D64.checkSvdMatrixSize(null,false,W,null,false);
        SingularOps_D64.checkSvdMatrixSize(null,true,W,V,true);
        SingularOps_D64.checkSvdMatrixSize(U,true,W,null,true);
    }
    ...
    public void nullVector() {
        for( int numRows = 2; numRows < 10; numRows++ ) {
            for( int numCols = 2; numCols < 10; numCols++ ) {
                ...
                SimpleMatrix v = SimpleMatrix.wrap(SingularOps_D64.nullVector(svd, true , null));
                ...
                v = SimpleMatrix.wrap(SingularOps_D64.nullVector(svd, false , null));
    ...
    public void rank_and_nullity( DenseMatrix64F A , int rank , int nullity ) {
        ...
        assertEquals(nullity, SingularOps_D64.nullity(alg, UtilEjml.EPS));
    }
    ...
    public void rank_and_nullity_noArgument( DenseMatrix64F A , int rank , int nullity ) {
        ...
        assertEquals(nullity, SingularOps_D64.nullity(alg));
    }
}
