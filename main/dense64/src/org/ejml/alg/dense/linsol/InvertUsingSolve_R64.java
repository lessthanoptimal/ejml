...
public class InvertUsingSolve_R64 {
    ...
    public static void invert(LinearSolver<DMatrixRow_F64> solver , D1MatrixRow_F64 A , DMatrixRow_F64 A_inv , DMatrixRow_F64 storage) {
        ...
        CommonOps_R64.setIdentity(storage);

        solver.solve(storage,A_inv);
    }

    public static void invert(LinearSolver<DMatrixRow_F64> solver , D1MatrixRow_F64 A , DMatrixRow_F64 A_inv ) {

        if( A.numRows != A_inv.numRows || A.numCols != A_inv.numCols) {
            throw new IllegalArgumentException("A and A_inv must have the same dimensions");
        }

        CommonOps_R64.setIdentity(A_inv);
