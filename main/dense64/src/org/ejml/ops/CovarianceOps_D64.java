...
public class CovarianceOps_D64 {
    ...
    public static int isValid( DenseMatrix64F cov ) {
        ...
        if( !MatrixFeatures_D64.isSymmetric(cov,TOL) )
            return 2;

        if( !MatrixFeatures_D64.isPositiveSemidefinite(cov) )
            return 3;
    ...
    public static boolean invert( final DenseMatrix64F cov , final DenseMatrix64F cov_inv ) {
        if( cov.numCols <= 4 ) {
            ...
            if( cov.numCols >= 2 )
                UnrolledInverseFromMinor_D64.inv(cov,cov_inv);
            else
     ...
     * covariance 'cov'.  If repeat calls are made to this class, consider using {@link CovarianceRandomDraw_D64} instead.
     *
