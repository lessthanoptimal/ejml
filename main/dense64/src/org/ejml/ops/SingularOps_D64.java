...
//        checkSvdMatrixSize(U, tranU, W, V, tranV);
                                            ...
                                            DenseMatrix64F nullSpace , double tol )
    {
        ...
        for( int i = 0; i < N; i++ ) {
            if( s[i] <= tol ) {
                CommonOps_D64.extract(V, i,i+1,0, V.numCols,nullSpace,count++,0);
            }
        }
        ...
        CommonOps_D64.transpose(nullSpace);
                                             ...
                                             DenseMatrix64F nullVector )
    {
        ...
        if( isRight )
            SpecializedOps_D64.subvector(A,smallestIndex,0,A.numRows,true,0,nullVector);
        else
            SpecializedOps_D64.subvector(A,0,smallestIndex,A.numRows,false,0,nullVector);
