...
//        eigenvalue -= eigenvalue*UtilEjml.EPS*10;
        ...
        SpecializedOps_D64.addIdentity(A,M,-eigenvalue);

        double threshold = NormOps_D64.normPInf(A)*UtilEjml.EPS;
        ...
        for( int i = 0; i < 200; i++ ) {
            ...
            if( MatrixFeatures_D64.hasUncountable(x)) {
                failed = true;
            }
            ...
            if( failed ) {
                ...
                b.set(x);
                NormOps_D64.normalizeF(b);

                // compute the residual
                CommonOps_D64.mult(M,b,x);
                double error = NormOps_D64.normPInf(x);
...
//                    CommonOps.set(b,1);
                    ...
                    // update everything
                    prevError = error;
                    eigenvalue = VectorVectorMult_D64.innerProdA(b,A,b);
                }
