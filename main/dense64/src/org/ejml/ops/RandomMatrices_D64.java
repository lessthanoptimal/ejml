 ...
 * @author Peter Abeles
 */
public class RandomMatrices_D64 {
...
//                System.out.println("j = "+j);
                if( j == 0 )
                    r = RandomMatrices_D64.createRandom(dimen,1,-1,1,rand);

                // find a vector that is normal to vector j
                // u[i] = (1/2)*(r + Q[j]*r)
                a.set(r);
                VectorVectorMult_D64.householder(-2.0,u[j],r,a);
                CommonOps_D64.add(r,a,a);
                CommonOps_D64.scale(0.5,a);
...
//                UtilEjml.print(a);
                ...
                double val = NormOps_D64.normF(r);
                if( val == 0 || Double.isNaN(val) || Double.isInfinite(val))
                    throw new RuntimeException("Failed sanity check");
                CommonOps_D64.divide(r,val);
            }
    ...
    public static DenseMatrix64F createInSpan( DenseMatrix64F[] span , double min , double max , Random rand ) {
        ...
        for( int i = 0; i < span.length; i++ ) {
            ...
            CommonOps_D64.scale(val,B);

            CommonOps_D64.add(A,B,A);
    ...
    public static DenseMatrix64F createSymmPosDef(int width, Random rand) {
        ...
        CommonOps_D64.multTransB(a,a,b);
