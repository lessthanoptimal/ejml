...
public class QRDecompositionHouseholderTran_R64 implements QRDecomposition<DMatrixRow_F64> {
    ...
    public DMatrixRow_F64 getQ(DMatrixRow_F64 Q , boolean compact ) {
        ...
        for( int j = minLength-1; j >= 0; j-- ) {
            ...
            QR.data[diagIndex] = 1;
            QrHelperFunctions_R64.rank1UpdateMultR(Q, QR.data, j * numRows, gammas[j], j, j, numRows, v);
    ...
    public void applyQ( DMatrixRow_F64 A ) {
        ...
        for( int j = minLength-1; j >= 0; j-- ) {
            ...
            double before = QR.data[diagIndex];
            QR.data[diagIndex] = 1;
            QrHelperFunctions_R64.rank1UpdateMultR(A, QR.data, j * numRows, gammas[j], 0, j, numRows, v);
            QR.data[diagIndex] = before;
        }
    }
    ...
    protected void householder( final int j )
    {
        ...
        final double max = QrHelperFunctions_R64.findMax(QR.data, startQR, numRows - j);
        ...
        if( max == 0.0 ) {
            ...
            tau = QrHelperFunctions_R64.computeTauAndDivide(startQR, endQR, QR.data, max);

            // divide u by u_0
            double u_0 = QR.data[startQR] + tau;
            QrHelperFunctions_R64.divideElements(startQR + 1, endQR, QR.data, u_0);
