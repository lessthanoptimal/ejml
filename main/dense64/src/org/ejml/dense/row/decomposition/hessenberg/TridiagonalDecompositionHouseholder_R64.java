...
public class TridiagonalDecompositionHouseholder_R64
    ...
    public TridiagonalDecompositionHouseholder_R64() {
        N = 1;
        w = new double[N];
        b = new double[N];
        gammas = new double[N];
    ...
    public DMatrixRow_F64 getQ(DMatrixRow_F64 Q , boolean transposed ) {
        ...
        if( transposed ) {
            for( int j = N-2; j >= 0; j-- ) {
                ...
                QrHelperFunctions_R64.rank1UpdateMultL(Q, w, gammas[j + 1], j + 1, j + 1, N);
            }
        } else {
            for( int j = N-2; j >= 0; j-- ) {
                w[j+1] = 1;
                ...
                QrHelperFunctions_R64.rank1UpdateMultR(Q, w, gammas[j + 1], j + 1, j + 1, N, b);
            }
        }
    ...
    private void similarTransform( int k) {
        ...
        if( max > 0 ) {
            ...
            double tau = QrHelperFunctions_R64.computeTauAndDivide(k, N, t, rowU, max);

            // write the reflector into the lower left column of the matrix
            double nu = t[rowU+k] + tau;
            QrHelperFunctions_R64.divideElements(k + 1, N, t, rowU, nu);
            t[rowU+k] = 1.0;
