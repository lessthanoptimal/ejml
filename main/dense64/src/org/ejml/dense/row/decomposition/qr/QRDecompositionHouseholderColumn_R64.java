...
import org.ejml.dense.row.decomposition.UtilDecompositons_R64;
 ...
 * </p>
 *
 * @see QRDecompositionHouseholder_R64
 *
 * @author Peter Abeles
 */
public class QRDecompositionHouseholderColumn_R64 implements QRDecomposition<DMatrixRow_F64> {
    ...
    public DMatrixRow_F64 getQ(DMatrixRow_F64 Q , boolean compact ) {
        if( compact ) {
            Q = UtilDecompositons_R64.checkIdentity(Q,numRows,minLength);
        } else {
            Q = UtilDecompositons_R64.checkIdentity(Q,numRows,numRows);
        }
        ...
        for( int j = minLength-1; j >= 0; j-- ) {
            ...
            double vv = u[j];
            u[j] = 1;
            QrHelperFunctions_R64.rank1UpdateMultR(Q, u, gammas[j], j, j, numRows, v);
            u[j] = vv;
        }
    ...
    public DMatrixRow_F64 getR(DMatrixRow_F64 R, boolean compact) {
        if( compact ) {
            R = UtilDecompositons_R64.checkZerosLT(R,minLength,numCols);
        } else {
            R = UtilDecompositons_R64.checkZerosLT(R,numRows,numCols);
        }
     ...
     */
    protected void convertToColumnMajor(DMatrixRow_F64 A) {
    ...
    protected void householder( int j )
    {
        ...
        final double max = QrHelperFunctions_R64.findMax(u, j, numRows - j);
        ...
        if( max == 0.0 ) {
            ...
            tau = QrHelperFunctions_R64.computeTauAndDivide(j, numRows, u, max);

            // divide u by u_0
            double u_0 = u[j] + tau;
            QrHelperFunctions_R64.divideElements(j + 1, numRows, u, u_0);
