...
public abstract class BaseLinearSolverQrp_R64 extends LinearSolverAbstract_R64 {
    ...
    public boolean setA(DMatrixRow_F64 A) {
        ...
        CommonOps_R64.extract(R, 0, rank, 0, rank, R11, 0, 0);

        if( norm2Solution && rank < numCols ) {
            // extract the R12 sub-matrix
            W.reshape(rank,numCols - rank);
            CommonOps_R64.extract(R,0,rank,rank,numCols,W,0,0);

            // W=inv(R11)*R12
            TriangularSolver_R64.solveU(R11.data, 0, R11.numCols, R11.numCols, W.data, 0, W.numCols, W.numCols);
