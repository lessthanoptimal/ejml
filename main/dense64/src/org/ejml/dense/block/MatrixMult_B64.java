...
import static org.ejml.dense.block.InnerMultiplication_B64.*;
...
//        checkInput( blockLength,A,B,C);
                                   ...
                                   D1Submatrix_F64 C )
    {
        ...
        if( !MatrixOps_B64.blockAligned(blockLength,A))
            throw new RuntimeException("Sub-Matrix A is not block aligned");

        if( !MatrixOps_B64.blockAligned(blockLength,B))
            throw new RuntimeException("Sub-Matrix B is not block aligned");

        if( !MatrixOps_B64.blockAligned(blockLength,C))
