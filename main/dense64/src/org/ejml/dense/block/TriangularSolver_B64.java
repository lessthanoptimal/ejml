...
import static org.ejml.dense.block.InnerMultiplication_B64.blockMultMinus;
 ...
 * @author Peter Abeles
 */
public class TriangularSolver_B64 {
                               ...
                               final double temp[] )
    {
        ...
        for( int i = 0; i < M; i += blockLength ) {
            ...
            for( int j = 0; j < i; j += blockLength ) {
                ...
                InnerTriangularSolver_B64.solveL(dataT,temp,heightT,widthX,heightT,indexII,0);
                System.arraycopy(temp,0,dataX,indexX,widthX*heightT);
            }
            InnerTriangularSolver_B64.invertLower(dataT,dataX,heightT,indexII,indexII);
        }
    }
                               ...
                               final double temp[] )
    {
        ...
        for( int i = 0; i < M; i += blockLength ) {
            ...
            InnerTriangularSolver_B64.invertLower(dataT,heightT,indexII);
        }
    }
            ...
            if( upper ) {
                ...
                if ( transT ) {
                    ...
                    for( int i = B.row0; i < B.row1; i += blockLength ) {
                        ...
                        InnerTriangularSolver_B64.solveLTransB(dataT,dataB,blockT_rows,N,blockT_rows,offsetT,offsetB);
                    }
                }
            }
        } else {
            ...
            if( upper ) {
                if ( transT ) {
                    for( int i = B.col0; i < B.col1; i += blockLength ) {
                        ...
                        InnerTriangularSolver_B64.solveTransU(dataT,dataB,Trows,N,Trows,offsetT,offsetB);
                    }
                } else {
                    for( int i = B.col0; i < B.col1; i += blockLength ) {
                        int offsetB = B.row0*B.original.numCols + Trows*i;
                        ...
                        int N = Math.min(B.col1 , i + blockLength ) - i;
                        InnerTriangularSolver_B64.solveU(dataT,dataB,Trows,N,Trows,offsetT,offsetB);
                    }
                }
            } else {
                if ( transT ) {
                    for( int i = B.col0; i < B.col1; i += blockLength ) {
                        ...
                        int N = Math.min(B.col1 , i + blockLength ) - i;
                        InnerTriangularSolver_B64.solveTransL(dataT,dataB,Trows,N,blockT_cols,offsetT,offsetB);
                    }
                } else {
                    for( int i = B.col0; i < B.col1; i += blockLength ) {
                        int offsetB = B.row0*B.original.numCols + Trows*i;
                        ...
                        int N = Math.min(B.col1 , i + blockLength ) - i;
                        InnerTriangularSolver_B64.solveL(dataT,dataB,Trows,N,blockT_cols,offsetT,offsetB);
                    }
                }
            }
        }
...
//                    Binner.row0 = Binner.row0;
                ...
                for( int k = B.col0; k < B.col1; k += blockLength ) {
                    ...
                    if( transL ) {
                        // Y = Y - T^T * B
                        MatrixMult_B64.multMinusTransA(blockLength, Linner,Binner,Y);
                    } else {

                        // Y = Y - T * B
                        MatrixMult_B64.multMinus(blockLength, Linner,Binner,Y);
                    }
                }
            }
        }
...
//                    Binner.row0 = Binner.row0;
                ...
                for( int k = B.col0; k < B.col1; k += blockLength ) {
                    ...
                    if( transR ) {
                        // Y = Y - T^T * B
                        MatrixMult_B64.multMinusTransA(blockLength, Rinner,Binner,Y);
                    } else {
                        // Y = Y - T * B
                        MatrixMult_B64.multMinus(blockLength, Rinner,Binner,Y);
                    }
                }
            }
        }
