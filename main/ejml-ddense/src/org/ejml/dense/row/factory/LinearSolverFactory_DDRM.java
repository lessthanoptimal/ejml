...
public class LinearSolverFactory_DDRM {
    ...
    /// Cholesky LDL solver for symmetric.
    ///
    /// @see org.ejml.dense.row.decomposition.chol.CholeskyDecompositionLDL_DDRM
    public static LinearSolverCholLDL_DDRM cholLdl() {
        return new LinearSolverCholLDL_DDRM();
    }

    /// Cholesky inner-product form
    ///
    /// @see org.ejml.dense.row.decomposition.chol.CholeskyDecompositionLDL_DDRM
    public static LinearSolverChol_DDRM cholInner( boolean lower ) {
        return new LinearSolverChol_DDRM(new CholeskyDecompositionInner_DDRM(lower));
    }

    /// Cholesky outer-product form block matrix. For large matrices. Converts input row-major
    /// matrix into a block matrix and processes that.
    ///
    /// @see org.ejml.dense.row.decomposition.chol.CholeskyDecompositionLDL_DDRM
    public static LinearSolverDense<DMatrixRMaj> cholOuterBlock( boolean lower ) {
        return new LinearSolverChol_DDRB(lower);
    }
    ...
    /// Solver for linear systems. Input matrix is N by N.
    ///
    ...
    /// then the basic solution is returned. See [org.ejml.dense.row.linsol.qr.BaseLinearSolverQrp_DDRM]
    ...
    ///
    /// See [#leastSquaresQrPivot] for additional options specific to QR decomposition based
