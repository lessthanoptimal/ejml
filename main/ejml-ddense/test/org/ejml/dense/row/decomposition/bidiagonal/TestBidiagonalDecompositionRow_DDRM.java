...
public class TestBidiagonalDecompositionRow_DDRM extends GenericBidiagonalCheck_DDRM {
    ...
    private void checkNaive(int m, int n) {
        ...
        assertTrue(decomp.decompose(A.ddrm().copy()));
        assertTrue(naive.decompose(A.ddrm()));
...
//        naive.getVTran().print();
        ...
        DMatrixRMaj foundA = U.mult(B).mult(V.transpose()).ddrm();

//        A.print();
//        foundA.print();

        assertTrue(MatrixFeatures_DDRM.isIdentical(A.ddrm(),foundA, UtilEjml.TEST_F64));
    }
