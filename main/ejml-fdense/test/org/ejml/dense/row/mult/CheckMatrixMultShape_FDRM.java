...
public class CheckMatrixMultShape_FDRM {

    // TODO merge with CheckMatrixMultShape_FSCC - Need to mess with project dependencies to do that

    Class theClass;

    public CheckMatrixMultShape_FDRM(Class theClass ) {
    ...
    private void checkPositive(Method func, boolean transA, boolean transB ,
        ...
        FMatrixRMaj A,B;
        FMatrixRMaj C = new FMatrixRMaj(m,o);

        if( transA ) {
            A = new FMatrixRMaj(n,m);
        } else {
            A = new FMatrixRMaj(m,n);
        }
        if( transB ) {
            B = new FMatrixRMaj(o,n);
        } else {
            B = new FMatrixRMaj(n,o);
        }

        TestMatrixMatrixMult_FDRM.invoke(func, 2.0f, A, B, C);

        if( UtilEjml.hasNullableArgument(func) ) {
            FMatrixRMaj ret = TestMatrixMatrixMult_FDRM.invoke(func, 2.0f, A, B, null);
            assertNotNull(ret);
...
//        System.out.println("func = "+func);
    ...
    private void checkNegative(Method func,
        ...
        FMatrixRMaj A,B;
        FMatrixRMaj C = new FMatrixRMaj(m_c,n_c);

        if( transA ) {
            A = new FMatrixRMaj(n_a,m_a);
        } else {
            A = new FMatrixRMaj(m_a,n_a);
        }
        if( transB ) {
            B = new FMatrixRMaj(n_b,m_b);
        } else {
            B = new FMatrixRMaj(m_b,n_b);
        }
