...
public class TestMatrixFeatures_CD64 {
    ...
    public void isNegative() {
        ...
        assertTrue(MatrixFeatures_CD64.isNegative(a,b,UtilEjml.TEST_F64));

        // now the negative case
        b.set(2,2,10,0);
        assertFalse(MatrixFeatures_CD64.isNegative(a,b,UtilEjml.TEST_F64));
    ...
    public void isEquals() {
        RowMatrix_C64 m = RandomMatrices_CD64.createRandom(3,4,-1,1,rand);
    ...
    public void isEquals_tol() {
        ...
        assertTrue(MatrixFeatures_CD64.isEquals(m,n,UtilEjml.TEST_F64));

        n.data[4] += UtilEjml.EPS;
        assertTrue(MatrixFeatures_CD64.isEquals(m,n,UtilEjml.TEST_F64));

        n.data[4] += Math.sqrt(UtilEjml.TEST_F64_SQ);
        assertFalse(MatrixFeatures_CD64.isEquals(m,n,UtilEjml.TEST_F64));
    ...
    public void isIdentity() {
        RowMatrix_C64 m = CommonOps_CD64.diag(1,0,1,0,1,0);

        assertTrue(MatrixFeatures_CD64.isIdentity(m,UtilEjml.TEST_F64));
    ...
    public void isHermitian() {
        ...
        assertFalse(MatrixFeatures_CD64.isHermitian(A, UtilEjml.TEST_F64));
    }

    @Test
    public void isUnitary() {
        // create a reflector since it's unitary
        RowMatrix_C64 u = RandomMatrices_CD64.createRandom(5,1,rand);
        Complex_F64 dot = new Complex_F64();
        VectorVectorMult_CD64.innerProdH(u, u,dot);
        double gamma = 2.0/dot.real;
        RowMatrix_C64 A = SpecializedOps_CD64.householder(u,gamma);

        assertTrue(MatrixFeatures_CD64.isUnitary(A, UtilEjml.TEST_F64));

        // try a negative case now
        A.set(0,1,495,400);

        assertFalse(MatrixFeatures_CD64.isUnitary(A, UtilEjml.TEST_F64));
    ...
    public void isUpperTriangle() {
        ...
        for( int hessenberg = 0; hessenberg < 2; hessenberg++ ) {
            ...
            for( int i = hessenberg; i < A.numRows; i++ ) {
                assertTrue(MatrixFeatures_CD64.isUpperTriangle(A,i,UtilEjml.TEST_F64));
            }

            // test negative
            for( int i = 0; i < hessenberg; i++ ) {
                assertFalse(MatrixFeatures_CD64.isUpperTriangle(A,i, UtilEjml.TEST_F64));
            }

            // see if it handles NaN well
            A.set(4,0,Double.NaN,Double.NaN);
            assertFalse(MatrixFeatures_CD64.isUpperTriangle(A,0,UtilEjml.TEST_F64));
        }
    }
    ...
    public void isLowerTriangle() {
        ...
        for( int hessenberg = 0; hessenberg < 2; hessenberg++ ) {
            ...
            // test positive
            for( int i = hessenberg; i < A.numRows; i++ ) {
                assertTrue(MatrixFeatures_CD64.isLowerTriangle(A,i,UtilEjml.TEST_F64));
            }

            // test negative
            for( int i = 0; i < hessenberg; i++ ) {
                assertFalse(MatrixFeatures_CD64.isLowerTriangle(A,i,UtilEjml.TEST_F64));
            }

            // see if it handles NaN well
            A.set(0,4,Double.NaN,Double.NaN);
            assertFalse(MatrixFeatures_CD64.isLowerTriangle(A,0,UtilEjml.TEST_F64));
        }
    }
