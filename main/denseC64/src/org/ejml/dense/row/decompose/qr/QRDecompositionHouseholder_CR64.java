 ...
 * @author Peter Abeles
 */
public class QRDecompositionHouseholder_CR64 implements QRDecomposition<DMatrixRow_C64> {
    ...
    public void setExpectedMaxSize( int numRows , int numCols ) {
        ...
        if( QR == null ) {
            QR = new DMatrixRow_C64(numRows,numCols);
    ...
    public DMatrixRow_C64 getQ(DMatrixRow_C64 Q , boolean compact ) {
        ...
        for( int j = minLength-1; j >= 0; j-- ) {
            QrHelperFunctions_CR64.extractHouseholderColumn(QR,j,numRows,j,u,0);
            QrHelperFunctions_CR64.rank1UpdateMultR(Q,u,0,gammas[j],j,j,numRows,v);
        }
    ...
    protected void householder( int j )
    {
        ...
        double max = QrHelperFunctions_CR64.extractColumnAndMax(QR,j,numRows,j,u,0);

        if( max <= 0.0 ) {
            gammas[j] = 0;
            error = true;
        } else {
            double gamma = QrHelperFunctions_CR64.computeTauGammaAndDivide(j,numRows,u,max,tau);
            gammas[j] = gamma;

            // divide u by u_0
            double real_u_0 = u[j*2]   + tau.real;
            double imag_u_0 = u[j*2+1] + tau.imaginary;
            QrHelperFunctions_CR64.divideElements(j + 1, numRows, u, 0, real_u_0,imag_u_0 );
            ...
            QrHelperFunctions_CR64.rank1UpdateMultR(QR,u,0,gamma,j+1,j,numRows,v);
     ...
     * @param A
     */
    protected void commonSetup(DMatrixRow_C64 A) {
