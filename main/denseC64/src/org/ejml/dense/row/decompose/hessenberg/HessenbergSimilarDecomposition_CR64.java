...
import org.ejml.dense.row.decompose.UtilDecompositons_CR64;
import org.ejml.dense.row.decompose.qr.QrHelperFunctions_CR64;
...
public class HessenbergSimilarDecomposition_CR64
        implements DecompositionInterface<DMatrixRow_C64> {
    // A combined matrix that stores te upper Hessenberg matrix and the orthogonal matrix.
    private DMatrixRow_C64 QH;
     ...
     */
    public HessenbergSimilarDecomposition_CR64(int initialSize) {
        gammas = new double[ initialSize ];
        b = new double[ initialSize*2 ];
        u = new double[ initialSize*2 ];
    }

    public HessenbergSimilarDecomposition_CR64() {
        this(5);
    }
     ...
     */
    public DMatrixRow_C64 getH(DMatrixRow_C64 H ) {
        H = UtilDecompositons_CR64.checkZeros(H,N,N);
     ...
     */
    public DMatrixRow_C64 getQ(DMatrixRow_C64 Q ) {
        Q = UtilDecompositons_CR64.checkIdentity(Q,N,N);

        Arrays.fill(u,0,N*2,0);
        for( int j = N-2; j >= 0; j-- ) {
            QrHelperFunctions_CR64.extractHouseholderColumn(QH,j+1,N,j,u,0);
            QrHelperFunctions_CR64.rank1UpdateMultR(Q, u, 0,gammas[j], j + 1, j + 1, N, b);
        }
    ...
    private boolean _decompose() {
        ...
        for( int k = 0; k < N-2; k++ ) { // k = column
            u[k*2] = 0;
            u[k*2+1] = 0;
            double max = QrHelperFunctions_CR64.extractColumnAndMax(QH,k+1,N,k,u,0);

            if( max > 0 ) {
                // -------- set up the reflector Q_k

                double gamma = QrHelperFunctions_CR64.computeTauGammaAndDivide(k+1,N,u,max,tau);
                gammas[k] = gamma;

                // divide u by u_0
                double real_u_0 = u[(k+1)*2]   + tau.real;
                double imag_u_0 = u[(k+1)*2+1] + tau.imaginary;
                QrHelperFunctions_CR64.divideElements(k + 2, N, u, 0, real_u_0,imag_u_0 );
                ...
                QrHelperFunctions_CR64.rank1UpdateMultR(QH, u,0, gamma, k + 1, k + 1, N, b);

                // ---------- multiply on the right by Q_k
                QrHelperFunctions_CR64.rank1UpdateMultL(QH, u,0, gamma, 0, k + 1, N);
