...
public class TridiagonalDecompositionHouseholder_CR64
    ...
    public TridiagonalDecompositionHouseholder_CR64() {
        N = 1;
        w = new double[N*2];
        b = new double[N*2];
        gammas = new double[N];
    ...
    public DMatrixRow_C64 getQ(DMatrixRow_C64 Q , boolean transposed ) {
        ...
        if( transposed ) {
            for( int j = N-2; j >= 0; j-- ) {
                QrHelperFunctions_CR64.extractHouseholderRow(QT,j,j+1,N,w,0);
                QrHelperFunctions_CR64.rank1UpdateMultL(Q, w, 0, gammas[j], j+1, j+1 , N);
            }
        } else {
            for( int j = N-2; j >= 0; j-- ) {
                QrHelperFunctions_CR64.extractHouseholderRow(QT,j,j+1,N,w,0);
                QrHelperFunctions_CR64.rank1UpdateMultR(Q, w, 0, gammas[j], j+1, j+1 , N, b);
            }
        }
    ...
    private void similarTransform( int k) {
        ...
        double max = QrHelperFunctions_CR64.computeRowMax(QT,k,k+1,N);

        if( max > 0 ) {
            double gamma = QrHelperFunctions_CR64.computeTauGammaAndDivide(k*N+k+1, k*N+N, t, max, tau);
            gammas[k] = gamma;

            // divide u by u_0
            double real_u_0 = t[(k*N+k+1)*2]   + tau.real;
            double imag_u_0 = t[(k*N+k+1)*2+1] + tau.imaginary;
            QrHelperFunctions_CR64.divideElements(k+2, N, t, k*N, real_u_0,imag_u_0 );
...
//            QrHelperFunctions_CR64.rank1UpdateMultR(QT,QT.data,k*N,gamma,k+1,k+1,N,w);
//            QrHelperFunctions_CR64.rank1UpdateMultL(QT,QT.data,k*N,gamma,k+1,k+1,N);
