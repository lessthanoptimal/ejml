...
public class BidiagonalDecompositionNaive_DDRM {
    ...
    protected void init(DMatrixRMaj A) {
        ...
        U = SimpleMatrix.identity(m, DMatrixRMaj.class);
        B = new SimpleMatrix(A);
        V = SimpleMatrix.identity(n, DMatrixRMaj.class);

        int max = Math.max(m,n);
        u = new DMatrixRMaj(max,1);
    }
    ...
    protected void computeU( int k) {
        ...
        if( max > 0 ) {
            ...
            SimpleMatrix Q_k = SimpleMatrix.wrap(SpecializedOps_DDRM.createReflector(this.u,nu/tau));
            U = U.mult(Q_k);
            B = Q_k.mult(B);
        }
    }
