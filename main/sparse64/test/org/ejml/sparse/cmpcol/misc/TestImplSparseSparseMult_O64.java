...
public class TestImplSparseSparseMult_O64 {
    ...
    @Test
    public void mult() {
        SMatrixCC_F64 a = RandomMatrices_O64.uniform(4,6,15,-1,1,rand);
        SMatrixCC_F64 b = RandomMatrices_O64.uniform(6,5,15,-1,1,rand);
        SMatrixCC_F64 c = RandomMatrices_O64.uniform(4,5,20,-1,1,rand);

        ImplSparseSparseMult_O64.mult(a,b,c,null);
        ...
        for (int row = 0; row < c.numRows; row++) {
            for (int col = 0; col < c.numCols; col++) {
                assertEquals(dense_c.get(row,col), c.get(row,col), UtilEjml.TEST_F64);
            }
        }
    }

    @Test
    public void multAddColA() {
        fail("implement");
    }
}