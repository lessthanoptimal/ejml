...
public class CheckMatrixMultShape_FSCC {

    // TODO merge with CheckMatrixMultShape_FDRM - Need to mess with project dependencies to do that

    Class theClass;

    public CheckMatrixMultShape_FSCC(Class theClass ) {
    ...
    private void checkPositive(Method func, boolean transA, boolean transB ,
        ...
        FMatrix A,B;
        FMatrix C = createMatrix(getMatrixType(func,2),m,o);
        ...
        invoke(func, 2.0f, A, B, C);

        if( hasNullableArgument(func) ) {
            FMatrix ret = invoke(func, 2.0f, A, B, null);
            assertNotNull(ret);
...
//        System.out.println("func = "+func);
    ...
    private void checkNegative(Method func,
        ...
        FMatrix A,B;
        FMatrix C = createMatrix(getMatrixType(func,2),m_c,n_c);
    ...
    public static FMatrix invoke(Method func,
                                 float alpha,
                                 FMatrix a, FMatrix b, FMatrix c)
        ...
        if( ret instanceof FMatrix )
            return (FMatrix)ret;
        return null;
    }

    public static FMatrix createMatrix( Class type , int rows , int cols ) {
