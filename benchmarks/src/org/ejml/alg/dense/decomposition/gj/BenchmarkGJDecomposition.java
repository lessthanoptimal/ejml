 ...
 * @author Peter Abeles
 */
public class BenchmarkGJDecomposition {


    public static void benchmark( GjPivotDecomposition<DenseMatrix64F> gauss , DenseMatrix64F orig , int numTrials ) {

        long prev = System.currentTimeMillis();
        for( long i = 0; i < numTrials; i++ ) {
            if( !gauss.decompose(orig) ) {
                throw new RuntimeException("Bad matrix");
            }
        }

        long time = System.currentTimeMillis() - prev;
        System.out.printf(" %20s time = %7d\n",gauss.getClass().getSimpleName(),time);
    }


    private static void runAlgorithms( DenseMatrix64F mat , int numTrials )
    {
        benchmark(new GjDecompositionRowPivot(),mat,numTrials);
    }
